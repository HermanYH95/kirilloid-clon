...
function changeRace(event) {
  ...
  row.querySelector(".hero_wrapper").style.display = raceUseHero(race) ? 'block' : 'none';
...
function changeHero() {
  var hero = this.checked;
  var row = findParent(this, 'tr');
  var idx = row.rowIndex - ROW_OFFSET;
  state.updateAtk(idx, 'hero', hero);
}
...
function add_row(data, idx) {
  ...
  row.querySelector(".hero").addEventListener('click', changeHero);
...
function del_row(event) {
  ...
  row.querySelector(".hero").removeEventListener('click', changeHero);
...
var state = {
  ...
  updateAtk: function(idx, field, value) {
    ...
    var delta = (this.bp ? -5 : 0) + (curr.bp ? +5 : 0) + (curr.hero ? +5 : 0);
